...
# Optional proxy support. Headers alone fix a "looks like a bot" block,
# but stats.nba.com is also known to block entire IP ranges used by
# cloud hosts (including Streamlit Community Cloud) at the network
# level -- no header can work around that, only routing the request
# through a different IP can. If headers alone don't fix "seasons
# won't load" after deploying, set NBA_PROXY_URL in Streamlit secrets
# (Settings -> Secrets) to a proxy URL, e.g.
#   NBA_PROXY_URL = "http://user:pass@proxy-host:port"
# and every request below will route through it automatically.
try:
    _PROXY_URL = st.secrets.get("NBA_PROXY_URL")
except Exception:
    _PROXY_URL = None
...
def get_player_info(player_id: int):
    ...
    try:
        info = commonplayerinfo.CommonPlayerInfo(
            player_id=player_id, timeout=_TIMEOUT, proxy=_PROXY_URL
        )
