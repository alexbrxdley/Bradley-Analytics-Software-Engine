...
# in a single tile. This is the main texture body.
NOISE_SVG = _noise_svg(base_frequency=0.85, octaves=5, gamma=1.4)

# Fine, high-frequency grain layered on top for crisp speckle detail.
NOISE_SVG_FINE = _noise_svg(base_frequency=1.9, octaves=2, gamma=1.3)
...
/* Aesthetic film grain -- two fixed noise layers at different scales,
   ...
   gradient (never darken it). Because feFuncA forces each tile fully
   opaque, the grain density is controlled cleanly by the gamma in the
   SVG plus the opacity here. */
.stApp::before {{
    content: "";
    position: fixed;
...
def get_shot_data(player_id: int, season: str, season_type: str):
    # 2 attempts at a 60s timeout: enough headroom for a slow-but-valid
    # response, but it surfaces a failure in a reasonable time (~2 min
    # worst case) instead of hanging for many minutes if the host is
    # blocked. If you're blocked, set NBA_PROXY (see fetch_with_retry).
